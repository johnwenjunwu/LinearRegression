...
def getBeta(train_x, train_y):
    ...
    xt = train_x.T
    beta = np.matmul(np.linalg.inv(np.matmul(xt, train_x)), np.matmul(xt, train_y))
    return beta
...
class LinearRegression(object):
    ...
    def bench(self):
        ...
        m.fit(self.train_x.values, self.train_y.values)
        p = m.predict(self.test_x.values)
        compute_mse(p, self.test_y.values)
    ...
    def linearModel(self):
        if(self.beta_type == 0):
            ...
            self.alpha = 0.005 # the size of samples is too small
